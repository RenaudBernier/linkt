...
export default function EmailVerificationPage() {
    ...
    useEffect(() => {
        // Redirect if no pending auth with EMAIL_VERIFICATION_REQUIRED
        // BUT don't redirect if we just successfully verified (flag prevents race condition)
        if (!pendingAuth || pendingAuth.status !== 'EMAIL_VERIFICATION_REQUIRED') {
    ...
    const handleCodeComplete = async (completedCode: string) => {
        ...
        try {
            ...
            // Check if response has token (verification successful)
            if (response.token) {
